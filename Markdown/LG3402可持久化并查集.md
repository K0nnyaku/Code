---
title: LG3402可持久化并查集
date: 2021-08-22 16:09:29
tags: [模板, 主席树, 可持久化数据结构, 并查集, 按秩合并]
---

### [LG3402](https://www.luogu.com.cn/problem/P3402)
### 注意按秩合并
```cpp
#include<bits/stdc++.h>
using namespace std;
...
void Union(int x, int y, int ver) {
    int sx = Ask(Rt[ver], 1, n, x)->size, sy = Ask(Rt[ver], 1, n, y)->size;
    if(sx > sy) swap(x, y);

    int fy = FindFa(y, ver);

    Cg(Rt[ver], 1, n, FindFa(x, ver))->val = fy;
